...
def get_data(report_filters):
	...
	# Get valuation rates for all raw materials
	item_codes = list(set([d.raw_material_item_code for d in work_orders if d.raw_material_item_code]))
	valuation_rates = get_valuation_rates(item_codes, report_filters.get("company"))
	...
	for d in work_orders:
		...
		# Add valuation rate and calculate values
		d.valuation_rate = valuation_rates.get(d.raw_material_item_code, 0.0)
		d.required_value = (d.required_qty or 0) * d.valuation_rate
		d.consumed_value = (d.consumed_qty or 0) * d.valuation_rate
		if d.required_qty:
			d.variance_percentage = (((d.consumed_qty or 0) - d.required_qty) / d.required_qty) * 100
		else:
			d.variance_percentage = 0.0
...
def get_valuation_rates(item_codes, company=None):
	"""Get valuation rates for items from Bin table"""
	if not item_codes:
		return {}

	valuation_rates = {}

	# Get valuation rate from Bin (average across all warehouses for the company)
	...
	# Calculate weighted average valuation rate per item
	item_totals = defaultdict(lambda: {"total_value": 0.0, "total_qty": 0.0})

	for bin_entry in bin_data:
		if bin_entry.valuation_rate and bin_entry.actual_qty:
			item_totals[bin_entry.item_code]["total_value"] += bin_entry.valuation_rate * bin_entry.actual_qty
			item_totals[bin_entry.item_code]["total_qty"] += bin_entry.actual_qty

	for item_code, totals in item_totals.items():
		if totals["total_qty"]:
			valuation_rates[item_code] = totals["total_value"] / totals["total_qty"]
		else:
			valuation_rates[item_code] = 0.0

	# For items not in Bin, try to get from Item master
	missing_items = [item for item in item_codes if item not in valuation_rates]
	if missing_items:
		item_valuation = frappe.get_all(
			"Item",
			filters={"name": ("in", missing_items)},
			fields=["name", "valuation_rate"],
		)
		for item in item_valuation:
			valuation_rates[item.name] = item.valuation_rate or 0.0
...
def get_columns():
	return [
		...
		{
			"label": _("Required Value"),
			"fieldname": "required_value",
			"fieldtype": "Currency",
			"width": 120,
		},
		{
